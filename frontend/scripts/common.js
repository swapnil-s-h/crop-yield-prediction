const statesAndDistricts = {
    ...
    "Puducherry": ["KARAIKAL", "MAHE", "PONDICHERRY", "YANAM"], "Punjab": ["AMRITSAR", "BARNALA", "BATHINDA", "FARIDKOT", "FATEHGARH SAHIB", "FAZILKA", "FIROZEPUR", "GURDASPUR", "HOSHIARPUR", "JALANDHAR", "KAPURTHALA", "LUDHIANA", "MANSA", "MOGA", "MUKTSAR", "NAWANSHAHR", "PATHANKOT", "PATIALA", "RUPNAGAR", "S.A.S NAGAR", "SANGRUR", "TARN TARAN"],
...
window.onload = function() {
    ...
    if (stateSelect) {
        ...
        Object.keys(statesAndDistricts).forEach((state, index) => {
            const option = document.createElement('option');
            option.value = index;  // state code (0,1,2...)
            option.text = state;
        ...
        stateSelect.addEventListener('change', function() {
            const selectedStateName = Object.keys(statesAndDistricts)[this.value];
            if (selectedStateName) {
                districtSelect.disabled = false;
                districtSelect.innerHTML = '';

                statesAndDistricts[selectedStateName].forEach((district, index) => {
                    const option = document.createElement('option');
                    option.value = index;  // district code (0,1,2...)
                    option.text = district;
                    districtSelect.appendChild(option);
                });
            } else {
                districtSelect.disabled = true;
                districtSelect.innerHTML = '<option>Please select state first</option>';
            }
        });
    }
};
